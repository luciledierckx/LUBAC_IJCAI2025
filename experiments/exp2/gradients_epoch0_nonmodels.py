...
def do_experiment(dico):
    ...
    nonmodel = PyLearner(input=uai, params=params, branching=None, outfolder=None, epsilon=None, jobs=jobs,
                        semiring=None, trainfile=tmp_f, testfile=None, learning_m=PyLearningMethod.NonModels, approx=PyApproximateMethod.LDS)
    print('nonmodel', nonmodel.evaluate(), nonmodel.get_epsilon(0))
    ...
    pred_nonmodel, _, grad_nonmodel, _ = nonmodel.eval_loss_grad([0], None, [nonmodel.get_expected()[0]])
    pred_exact, _, grad_exact, _ = exact.eval_loss_grad(None, None, exact.get_expected())
    
    
    with open(output_file, "a") as out:
        out.write(str(v)+","+'{:.5e}'.format(pred_nonmodel[0])+","+'{:.5e}'.format(pred_exact[0])+","
                +str(' '.join([' '.join(['{:.5e}'.format(elem) if elem!=0 else str(elem) for elem in v]) for v in grad_nonmodel[0]])+",")
                +str(' '.join([' '.join(['{:.5e}'.format(elem) if elem!=0 else str(elem) for elem in v]) for v in grad_exact[0]]))+','
                +str(' '.join([' '.join(['{:.5e}'.format(elem) if elem!=0 else str(elem) for elem in v]) for v in nonmodel.get_distributions()]))+
                "\n")
