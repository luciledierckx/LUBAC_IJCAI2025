...
def do_experiment(dico):
    ...
    output_folder = "/experiments/data/outputs_LDS/bayesian_networks/"+dataset+"/grad_e0_comparison_models/t"+str(timeout)+'/'
    ...
    model = PyLearner(input=uai, params=params, branching=None, outfolder=None, epsilon=None, jobs=jobs,
                    semiring=None, trainfile=tmp_f, testfile=None, learning_m=PyLearningMethod.Models, approx=PyApproximateMethod.LDS)
    print('model', model.evaluate(), model.get_epsilon(0))
    ...
    with open(output_file, "w") as out:
        out.write("expected,pred,exact,grad,grad_exact,weights\n")
    
    pred_model, _, grad_model, _ = model.eval_loss_grad([0], None, [model.get_expected()[0]])
    pred_exact, _, grad_exact, _ = exact.eval_loss_grad(None, None, exact.get_expected())
    
    
    with open(output_file, "a") as out:
        out.write(str(v)+","+'{:.5e}'.format(pred_model[0])+","+'{:.5e}'.format(pred_exact[0])+","
                +str(' '.join([' '.join(['{:.5e}'.format(elem) if elem!=0 else str(elem) for elem in v]) for v in grad_model[0]])+",")
                +str(' '.join([' '.join(['{:.5e}'.format(elem) if elem!=0 else str(elem) for elem in v]) for v in grad_exact[0]]))+','
                +str(' '.join([' '.join(['{:.5e}'.format(elem) if elem!=0 else str(elem) for elem in v]) for v in model.get_distributions()]))+
                "\n")
